...
def create_payment_intent(request):
    ...
    try:
        with transaction.atomic():
            ...
            # If payment already exists and is PENDING, return existing intent
...
def payment_webhook(request):
    ...
    try:
        if event["type"] == "payment_intent.succeeded":
            ...
            if order_id:
                try:
                    order = Order.objects.select_related("listing").get(pk=int(order_id))
                    if order.status == Order.Status.ACCEPTED:
        ...
        # Log but don't crash — always return 200 to Stripe
        import logging
